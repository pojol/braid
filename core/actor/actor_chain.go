...
func (sh *ScriptHandler) Execute(m *router.MsgWrapper) error {
	v, err := sh.interpreter.Eval("Execute")
	if err != nil {
		return fmt.Errorf("failed to get Execute function: %w", err)
	}

	fn, ok := v.Interface().(func(*router.MsgWrapper) error)
	if !ok {
